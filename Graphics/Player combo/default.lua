...
local function worstJudgeClear()
	if (ScoringInfo and ScoringInfo.worstJudge) then
		...
		if (ScoringInfo.seed ~= GAMESTATE:GetStageSeed()) or
			(tns_reverse[wj] < tns_reverse['TapNoteScore_W4'])
		then
...
local t = Def.ActorFrame {
	...
	ComboCommand=function(self, param)
		...
		elseif decideColor('TapNoteScore_W4', param) then
			cfShowOnly('NumberW4', Label('LabelW4',param));
